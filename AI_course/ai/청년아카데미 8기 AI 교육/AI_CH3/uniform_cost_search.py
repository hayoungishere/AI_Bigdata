...
class UniformCostSearch(util.SearchAlgorithm):
    ...
    def solve(self, problem):
        ...
        explored = set() # Explored states, Can't saved Duplicated Value.
        backpointers = {}  # map state to (action, previous state)
        frontier.update(problem.start_state(), 0) # input Start_State
        ...
        while not frontier.is_empty():            
            ...
            if problem.is_end(state):
                totalCost = priority
                actions = []
                while state != problem.start_state(): # Recovery PATH from start point to end point
